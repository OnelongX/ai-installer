...
| 步骤 | 内容 |
|---|---|
| ① 检测 | 检查 Node.js、Codex CLI、`~/.codex/config.toml` 是否就位 |
| ② 装 Node | 缺则用 `winget install OpenJS.NodeJS.LTS` 自动装上 |
| ③ 装 CLI | `npm i -g @openai/codex` |
| ④ 写 Key | `OPENAI_API_KEY` 写入 Windows 用户环境变量 |
| ⑤ 写配置 | 在 `%USERPROFILE%\.codex\config.toml` 写好默认模型、provider、base_url |
| ⑥ 验证 | `codex --version` 跑通即成功 |
...
| 类型 | 文件 | 适用场景 |
|---|---|---|
| 便携版 | [`Codex-Installer-Portable-0.1.4.exe`](https://github.com/OnelongX/ai-installer/releases/download/codex-v0.1.4/Codex-Installer-Portable-0.1.4.exe) | 双击即用，不写注册表 |
| 安装包 | [`Codex-Installer-Setup-0.1.4.exe`](https://github.com/OnelongX/ai-installer/releases/download/codex-v0.1.4/Codex-Installer-Setup-0.1.4.exe) | 标准 NSIS 安装到 Program Files，带桌面快捷方式 |
...
安装完成后 `%USERPROFILE%\.codex\config.toml`：
...
A：这是 Node 老版本 spawn 在 Windows 上的引号 bug。本工具内部已修，正常不会再出现。

**Q：装完跑 `codex` 提示找不到命令？**
A：必须开**新**终端窗口。环境变量更新只对新进程生效。

**Q：要换 base URL 或换模型？**
A：直接编辑 `%USERPROFILE%\.codex\config.toml`。Codex CLI 启动时读这个文件。

**Q：怎么卸？**
A：
1. NSIS 版从控制面板"添加或删除程序"卸载工具本身
2. CLI 卸载：`npm uninstall -g @openai/codex`
3. 清配置：删 `%USERPROFILE%\.codex\` 目录
4. 清 Key：PowerShell 跑 `[Environment]::SetEnvironmentVariable('OPENAI_API_KEY', $null, 'User')`
...
仓库里附带了一个 [`install_codex_macos.py`](./install_codex_macos.py)，让 macOS 用户走纯命令行流程。
...
- 检查 macOS 环境
- 缺 `node` / `npm` 时自动下载并安装官方 Node.js `pkg`
- 强制要求 `OPENAI_API_KEY`
- 安装 `@openai/codex`
