...
describe('app startup recovery', () => {
  ...
  it('shows a recovery prompt when an interrupted session exists and can dismiss it', async () => {
      ...
      <App
        installerClient={{
          ...
          getRecoveryState: async () => ({
            ...
            failedTaskId: 'install-codex',
            lastFailureAt: '2026-04-23T09:30:00.000Z',
            pendingTasks: ['install-codex', 'verify-codex-runtime'],
            recentLogs: ['Write C:\\Users\\Administrator\\.codex\\config.toml', 'network timed out'],
            summary: '上一次安装在下载 Codex 时中断。'
          }),
    ...
    expect(await screen.findByText('检测到未完成的 Codex 安装。')).toBeInTheDocument()
    expect(screen.getByText('上次失败任务：')).toBeInTheDocument()
    expect(screen.getByText('安装 Codex CLI')).toBeInTheDocument()
    expect(screen.getByText('已完成任务：')).toBeInTheDocument()
    expect(screen.getByText('写入 Codex 配置')).toBeInTheDocument()
    expect(screen.getByText('待执行任务：')).toBeInTheDocument()
    expect(screen.getByText('安装 Codex CLI, 验证 Codex 运行环境')).toBeInTheDocument()
