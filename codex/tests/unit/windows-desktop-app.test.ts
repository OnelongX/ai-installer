...
describe('windows desktop app detection', () => {
  ...
  it('detects the app from a start menu shortcut when the exe path is not in common locations', async () => {
    const startMenuShortcut =
      'C:\\Users\\Administrator\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\Codex.lnk'
    ...
    expect(result.statusMessage).toContain('Codex.lnk')
  })

  it('detects the app from uninstall registry output when file probes miss', async () => {
    const installLocation = 'C:\\Users\\Administrator\\AppData\\Local\\Programs\\Codex'
    ...
    const result = await detectWindowsDesktopApp({
      ...
      fileExists: async (targetPath) => targetPath === `${installLocation}\\Codex.exe`
    })

    expect(result.installed).toBe(true)
    expect(result.launchPath).toBe(`${installLocation}\\Codex.exe`)
  })
  ...
  it('returns the official install guidance when the app is missing', async () => {
    ...
    expect(result.downloadUrl).toBe(codexDesktopDownloadUrl)
    expect(result.statusMessage).toBe('未检测到 Codex 桌面版，可继续前往官方页面安装。')
  })
})
