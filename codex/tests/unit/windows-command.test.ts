...
describe('windows command resolution', () => {
  ...
  it('finds the npm-installed Codex shim in AppData', () => {
    const env = {
      APPDATA: 'C:\\Users\\Administrator\\AppData\\Roaming',
      LOCALAPPDATA: 'C:\\Users\\Administrator\\AppData\\Local'
    }

    const paths = getKnownWindowsCommandPaths('codex', env, (candidate) => {
      return candidate === path.join(env.APPDATA, 'npm', 'codex.cmd')
    })

    expect(paths).toEqual([path.join(env.APPDATA, 'npm', 'codex.cmd')])
  })
  ...
  it('marks cmd shims to run through the Windows shell', () => {
    ...
    const invocations = getCommandInvocations('codex', ['--version'], 'win32', env, (candidate) => {
      return candidate === path.join(env.APPDATA, 'npm', 'codex.cmd')
    })
    ...
    expect(invocations).toEqual([
      ...
      {
        args: ['--version'],
        command: path.join(env.APPDATA, 'npm', 'codex.cmd'),
        shell: true
      }
    ])
  })
