...
const fallbackPlan: InstallPlan = {
  ...
  tasks: ['install-node', 'install-codex', 'write-config']
}
...
const fallbackDesktopAppState: DesktopAppStateData = {
  ...
  installed: false,
  statusMessage: '正在检查 Codex 桌面版是否已安装。'
}
...
export default function App({ installerClient }: AppProps) {
    ...
    return (
      <>
        ...
        <RecoveryPrompt
          ...
          onResume={() => {
            setExecutionState({
              currentTask: recoveryState.failedTaskId ?? recoveryState.pendingTasks[0] ?? 'install-codex',
        ...
        <ApiKeyStep
          ...
          onContinue={({ apiKeyMode, keyValue }) => {
            const continueWithEnvironment = async () => {
              ...
              const hasCodex = environment.some(
                (item) => item.id === 'codex' && item.status === 'satisfied'
              )
              const nextPlan = await client.generatePlan({
                apiKeyMode,
                codexInstalled: hasCodex,
              ...
              .then((opened) => {
                ...
                setDesktopAppState((current) => ({
                  ...
                  statusMessage: opened
                    ...
                    : '打开安装页面失败，请手动访问 https://openai.com/codex/get-started/'
                }))
              })
              .catch(() => {
                setDesktopAppActionLabel(undefined)
                setDesktopAppState((current) => ({
                  ...current,
                  statusMessage: '打开安装页面失败，请手动访问 https://openai.com/codex/get-started/'
                }))
              })
          }}
          onOpenDesktopApp={() => {
            ...
            void client
              ...
              .catch(() => {
                ...
                setDesktopAppState((current) => ({
                  ...current,
                  statusMessage: '启动 Codex 桌面版失败，请手动打开已安装的应用。'
                }))
              })
          }}
          onOpenProviderSite={() => {
    ...
    <>
      ...
      <PlanView
        ...
        onStart={async () => {
          ...
          setExecutionState({
            currentTask: activePlan.tasks[0] ?? 'install-codex',
            failedTask: undefined,
            issueMessage: undefined,
            issueUserAction: undefined,
            logs: [],
  ...
  function handleExecutionResult(result: InstallExecutionResult) {
    if (result.status === 'failed') {
      setExecutionState((current) => ({
        currentTask: current.currentTask || plan?.tasks.at(-1) || 'verify-codex-runtime',
        failedTask: current.failedTask ?? current.currentTask ?? 'verify-codex-runtime',
    ...
    setExecutionState((current) => ({
      currentTask: plan?.tasks.at(-1) ?? 'verify-codex-runtime',
      failedTask: undefined,
      issueMessage: undefined,
      issueUserAction: undefined,
      logs: mergeLogs(current.logs, result.logs),
...
function getProgressLabel(event: InstallLogEvent) {
  ...
  switch (event.type) {
    case 'task-complete':
      return event.taskId === 'verify-codex-runtime' ? '安装完成' : `已完成 ${taskLabel}`
    case 'task-failed':
      return '安装失败'
    case 'task-start':
